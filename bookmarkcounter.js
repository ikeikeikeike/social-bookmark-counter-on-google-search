...
function twitterCountView(link) {
  ...
  GM_xmlhttpRequest({
    ...
    onload: function(res) {
      ...
      if (!json)
        return;
      var total = json.response.trackback_total;
      if (total < 1)
        return;
...
function facebookCountView(link) {
  var encode = encodeURI(link.href);
  var url = "https://api.facebook.com/method/fql.query?query=select like_count, total_count, share_count, comment_count, normalized_url, url from link_stat where url='" + encode + "'&format=json&callback=crossdomain_res";
  ...
  GM_xmlhttpRequest({
    ...
    onload: function(res) {
      ...
      if (!res.responseText)
        return;
      var json = eval(res.responseText.replace('crossdomain_res', ''))[0];
      if (json.length < 1)
        return;
